...
<body>
				...
				<c:when test="${not empty notes}">
					<c:forEach items="${notes}" var="n" varStatus="i">
						<div class="layui-colla-item">
							<ul class="my-content">
								<li>${i.index + 1}</li>
								<li title="${n.title }">${n.title }
									<div id="content${n.id }" style="display: none;">${n.content }</div>
								<li>${n.createdTime }</li>
								<li><img onclick="view('${n.id }')" title="查看"
									src="${pageContext.request.contextPath}/images/view.png">
									<img onclick="edit('${n.id }')" title="编辑"
									src="${pageContext.request.contextPath}/images/edit.png">
									<img onclick="exportworld('${n.id }','${n.title }')"
									title="导出成world"
									src="${pageContext.request.contextPath}/images/export.png">
									<img onclick="del('${n.id }')" title="删除"
									src="${pageContext.request.contextPath}/images/del.png">
								</li>
							</ul>
						</div>
					</c:forEach>
	...
	<script
		src="${pageContext.request.contextPath}/js/jquery.wordexport.js"></script>
	<script type="text/javascript">
		...
		function del(id) {
			if (confirm("是否确认删除？")) {
				location.href = "${pageContext.request.contextPath}/note/del.action?&id="
						+ id;
			}
		}
		function view(id) {
			location.href = "${pageContext.request.contextPath}/note/view.action?&id="
					+ id;
		}
		function edit(id) {
			location.href = "${pageContext.request.contextPath}/note/edit.action?&id="
					+ id;
		}
	</script>
</body>
